...
public class AnimationFileReader {
    ...
    while (sc.hasNext()) {
      ...
      switch (command) {
        case "rectangle":
          ...
          break;
        case "rectangle0":
          RectangleZeroInfo info = readRectangleZeroInfo(sc);
          builder.addRectangle(
                  info.getName(),
                  info.getX(), info.getY(),
                  info.getWidth(), info.getHeight(),
                  info.getR(), info.getG(), info.getB(),
                  info.getStart(), info.getEnd(), 1);
          break;
        case "oval":
          ...
          break;
        case "oval0":
          OvalZeroInfo cinfonol = readOvalZeroInfo(sc);
          builder.addOval(
                  cinfonol.getName(),
                  cinfonol.getX(), cinfonol.getY(),
                  cinfonol.getXRadius(), cinfonol.getYRadius(),
                  cinfonol.getR(), cinfonol.getG(), cinfonol.getB(),
                  cinfonol.getStart(), cinfonol.getEnd(), 1);
          break;
        ...
        case "scale":
          ...
          case "background":
          BackgroundInfo backgroundInfo = readBackgroundInfo(sc);
          builder.addBackground(backgroundInfo.getRed(),
                  backgroundInfo.getGreen(), backgroundInfo.getBlue());
          break;
        default:
  ...
  private BackgroundInfo readBackgroundInfo(Scanner sc) throws
          IllegalStateException, InputMismatchException {
    BackgroundInfo info = new BackgroundInfo();
    while (!info.isAllInitialized()) {
      info.setRed(sc.nextInt());
      info.setGreen(sc.nextInt());
      info.setBlue(sc.nextInt());
  ...
  private RectangleZeroInfo readRectangleZeroInfo(Scanner sc) throws
          IllegalStateException, InputMismatchException {
    RectangleZeroInfo info = new RectangleZeroInfo();
    while (!info.isAllInitialized()) {
      String command = sc.next();
      switch (command) {
        case "lower-left-x":
  ...
  private OvalZeroInfo readOvalZeroInfo(Scanner sc) throws
          IllegalStateException, InputMismatchException {
    OvalZeroInfo info = new OvalZeroInfo();
  ...
  //***************************************
  class RectangleZeroInfo extends ShapeInfo {
    private float x;
    private float y;
    private float width;
    private float height;
  ...
  class OvalZeroInfo extends ShapeInfo {
    private float cx;
    private float cy;
    private float xradius;
    private float yradius;
  ...
  class BackgroundInfo extends Inputable {
    private int red;
    private int green;
    private int blue;
